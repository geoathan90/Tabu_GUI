...
def solve_horizontal_tensions_legacy(
    ...
    """
    Legacy-C-style outer solve on H0.

    Mirrors the old TANS5 logic:
      - start from H0 = T_ref
      - keep a step size dt (starts at 99)
      - if solved total length is too short, decrease H0 by dt
      - if solved total length is too long, increase H0 by dt
      - whenever the direction flips, halve dt
      - stop when abs(total_length - target_total_length) <= atol_m

    This is intentionally not a generic root finder. It is here to reproduce
    the legacy C convergence path as closely as possible.
    """
    N = len(spans)
    target = float(np.sum(lengths(np.full(N, float(T_ref), dtype=float))))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conductor", default="Cardinal")
    parser.add_argument("--ruling", type=int, default=500)
    parser.add_argument("--atol", type=float, default=0.01)
    parser.add_argument("--out-dir", default="outputs")
    parser.add_argument("--solver", default="Legacy")
